...
let init=0
function Header({setSearchResults,searchType}) {
  ...
  <>
    ...
    <div className='relative py-5 px-2 z-10 bg-gray-900 '>
        ...
        <div className='relative w-full lg:w-1/2 border-2 order-last lg:order-none  border-white rounded-md'>
            <input type='text' onChange={onChangeSearchHandler}  ref={searchRef} placeholder='What do you want to watch?' className='w-full bg-transparent p-2 text-white rounded-lg'/>
        ...
        <div className='flex w-fit gap-5 lg:py-0 py-2 justify-center font-semibold'>
          {user.isAuthenticated?<UserComponent/>:  <button className='text-white' onClick={()=>{dispatch(logIn())}}>Sign in</button>}
